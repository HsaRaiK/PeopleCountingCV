...
def processndraw(frame, out, timer):
    ...
    for o in out:
        for detected in o:
            if detected[4] > objth:
                ...
                conf = scores[classid]
                if conf > confth and classes[classid] == 'person':
    ...
    for i in ind:
        ...
        cv2.putText(frame, 'person', (box[0], box[1]), cv2.FONT_HERSHEY_COMPLEX, 0.65, (255, 255, 0), 2)
...
outputFile = "TrackedYoloKCF1"
outv= cv2.VideoWriter(outputFile, cv2.VideoWriter_fourcc('M','J','P','G'), 30, (round(cap.get(cv2.CAP_PROP_FRAME_WIDTH)),round(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))))
count=0
tflag=0
fdflag=0
while(cap.isOpened()):
